...
//   describe("Blacklisting and Trading Control", function () {
//     it("Should revert transfer if sender or recipient is blacklisted", async () => {
//       const { cloudax, owner, john } = await loadFixture(deployAndSetup);
//       await cloudax.connect(owner).setBlacklisted(john.address, true);
//       await expect(cloudax.connect(john).transfer(owner.address, 100)).to.be
//         .revertedWithCustomError;
...
//     it("Should revert transferFrom if sender or recipient is blacklisted", async () => {
//       const { cloudax, owner, john, jane } = await loadFixture(deployAndSetup);
//       await cloudax.connect(owner).setBlacklisted(john.address, true);
//       await expect(
//         cloudax.connect(john).transferFrom(owner.address, jane.address, 100)
//       ).to.be.revertedWithCustomError;
//     });

//     it("Should revert transfer if trading is disabled and sender is not owner or presale address", async () => {
//       const { cloudax, owner, john } = await loadFixture(deployAndSetup);
//       await expect(cloudax.connect(john).transfer(owner.address, 100)).to.be
//         .revertedWithCustomError;
//     });

//     it("Should revert transferFrom if trading is disabled and sender is not owner or presale address", async () => {
//       const { cloudax, owner, john, jane } = await loadFixture(deployAndSetup);
//       await expect(
//         cloudax.connect(john).transferFrom(owner.address, jane.address, 100)
//       ).to.be.revertedWithCustomError;
...
//     it("Should allow transfer if trading is disabled but sender is owner or presale address", async () => {
//       const { cloudax, owner, john } = await loadFixture(deployAndSetup);
//       await cloudax.connect(owner).setTradingEnabled(false);
//       await cloudax.connect(owner).transfer(john.address, 100);
//       expect(await cloudax.balanceOf(john.address)).to.equal(100);
//     });

//     it("Should allow transferFrom if trading is disabled but sender is owner or presale address", async () => {
//       const { cloudax, owner, john, jane } = await loadFixture(deployAndSetup);
//       // await cloudax.connect(owner).setTradingEnabled(false);

//       // Approve john to spend 100 tokens on behalf of the owner
//       await cloudax.connect(owner).approve(john.address, 200);

//       // Now john can transferFrom 100 tokens from the owner to jane
//       await cloudax
//         .connect(john)
//         .transferFrom(owner.address, jane.address, 100);

//       // Check that jane's balance has increased by 100
//       expect(await cloudax.balanceOf(jane.address)).to.equal(100);
//     });
//   });
// });
