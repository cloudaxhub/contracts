...
/**
 * @title CloudaxTeamVestingWallet (smart contract)
 ...
 *
 * Features:
 * - Manage the vesting schedules for team allocated token
 ...
 *
 * * Components:
 * - Contract: `CloudaxTeamVestingWallet`, which extends `Ownable`, `ReentrancyGuard`, and `Pausable` to manage the vesting of tokens for the Cloudax team.
 * - Key Functions:
...
contract CloudaxTeamVestingWallet is Ownable2Step, ReentrancyGuard, Pausable {
     ...
     * @param token_ The address of the BEP20 token contract.
     */
    ...
    function initialize(address beneficiary_) external onlyOwner {
        ...
        uint256 userAllocation = ((_token.totalSupply() * 10) / 100);
